elementBody.appendChild(parseFunction(`<div id="divWindowAdd" style="z-index: 5">
...
const deleteInputWithNullValue = (event) =>{
    ...
    event.target.value.length===0?allElem.forEach((e,i) =>i<allElem.length-1&&e.value.length===0?e.remove():-1):-1;
};

const addTask =()=>{
    let parent = document.querySelector('.taskList');
    let lastInput = parent.lastElementChild;
    lastInput.value.length!==0?(document.querySelector('.taskList').appendChild(
            parseFunction(`<input type="text" placeholder=" Add to-do" class="newTask" style="outline: none;" 
               oninput="deleteInputWithNullValue(event)" onchange="addTask()" value="">`)),
            parent.lastElementChild.focus()):-1;
};
...
const btnAddNewList = () =>{
    ...
    let inputs = document.querySelectorAll(".newTask");
    inputs = Array.from(inputs).map((e,i) =>e.value!==''?e.value:e.value='to-do');
...
const addNewToDoBoard = (newTitle,arrayNewTasks) => {
    ...
    arrayNewTasks.forEach(i =>newIdTask.push(randomInteger(1, 1000000)));
    // toDoBoard = {...toDoBoard,toDoBoard:{idList:newIdList,title:newTitle,tasks:[]}};
...
function randomInteger(min, max) {
    ...
    toDoBoard.forEach(e=>e.idList === rand?randomInteger(min, max):e.tasks.forEach(i =>
